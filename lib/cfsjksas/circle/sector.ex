defmodule Cfsjksas.Circle.Sector do
    @moduledoc """
  ...
  #@spec make_shape(integer(), integer(), String.t(), String.t(), String.t()) :: Cfsjksas.Circle.Sector.t()
  ...
  #@spec new_struct(integer(), integer(), String.t(), String.t(), String.t()) :: Cfsjksas.Circle.Sector.t()
  defp new_struct(gen, sector_num, line_color, fill, fill_opacity) do
    %Cfsjksas.Circle.Sector{gen: gen, sector_num: sector_num,
      line_color: line_color, fill: fill, fill_opacity: fill_opacity}
  end

  #@spec add_id(Cfsjksas.Circle.Sector.t()) :: Cfsjksas.Circle.Sector.t()
  defp add_id(sector) do
    id = "sector_" <> to_string(sector.gen) <> "_" <> to_string(sector.sector_num)
    %Cfsjksas.Circle.Sector{sector | id: id}
  end
  ...
  defp add_quadrant(sector) do
    ...
    %Cfsjksas.Circle.Sector{sector | quadrant: quadrant}
  end

  defp add_reverse(sector) do
    %Cfsjksas.Circle.Sector{sector | reverse: quadrant_reverse(sector.quadrant)}
  end
  ...
  defp add_inner_radius(sector) do
    inner_radius = Cfsjksas.Circle.Get.radius(sector.gen - 1)
    %Cfsjksas.Circle.Sector{sector | inner_radius: inner_radius }
  end

  defp add_outer_radius(sector) do
    outer_radius = Cfsjksas.Circle.Get.radius(sector.gen)
    %Cfsjksas.Circle.Sector{sector | outer_radius: outer_radius }
  end

  defp add_lower_radians(sector) do
    lower_radians = sector.sector_num * 2 * :math.pi() / (2 ** sector.gen)
    %Cfsjksas.Circle.Sector{sector | lower_radians: lower_radians }
  end

  defp add_upper_radians(sector) do
    upper_radians = rem(sector.sector_num + 1, 2**sector.gen) * 2 * :math.pi() / (2 ** sector.gen)
    %Cfsjksas.Circle.Sector{sector | upper_radians: upper_radians }
  end

  defp add_points(sector) do
    # point a is outer_radius, upper_radians
    {a_x, a_y} = Cfsjksas.Circle.Get.xy(sector.outer_radius, sector.upper_radians)
    # point b is outer_radius, lower_radians
    {b_x, b_y} = Cfsjksas.Circle.Get.xy(sector.outer_radius, sector.lower_radians)
    # point c is inner_radius, upper_radians
    {c_x, c_y} = Cfsjksas.Circle.Get.xy(sector.inner_radius, sector.upper_radians)
    # point d is inner_radius, lower_radians
    {d_x, d_y} = Cfsjksas.Circle.Get.xy(sector.inner_radius, sector.lower_radians)

    %Cfsjksas.Circle.Sector{sector | a_x: a_x, a_y: a_y,
                                b_x: b_x, b_y: b_y,
                                c_x: c_x, c_y: c_y,
                                d_x: d_x, d_y: d_y }
  end
  ...
  defp add_svg(sector) when sector.gen == 0 do
    ...
    {ctr_x, ctr_y} = Cfsjksas.Circle.Get.center
    r = Cfsjksas.Circle.Get.radius(sector.gen)

    # get necessary info
    config = Cfsjksas.Circle.Get.config(sector.gen)
    ...
    %Cfsjksas.Circle.Sector{sector | svg: svg}
  end
...
def add_name(gen, person, sector) when gen in 1..2 do
  ...
  sweep = Cfsjksas.Circle.Get.quadrant_sweep(sector.quadrant)
  font_size = Cfsjksas.Circle.Get.config(gen).font_size
...
def add_name(gen, person, sector) when gen == 3 do
  ...
  config = Cfsjksas.Circle.Get.config(gen)
...
def add_name(gen, person, sector) when gen in 7..14 do
  ...
  {inner_x, inner_y} = Cfsjksas.Circle.Get.xy(r_inner, phi)
  {outer_x, outer_y} = Cfsjksas.Circle.Get.xy(r_outer, phi)
...
defp arc_ends(quadrant, r, angle1, angle2)
  ...
  {beg_x, beg_y} = Cfsjksas.Circle.Get.xy(r, angle1)
  {end_x, end_y} = Cfsjksas.Circle.Get.xy(r, angle2)
...
defp arc_ends(quadrant, r, angle1, angle2)
  ...
  # so that words have right orientation
  {beg_x, beg_y} = Cfsjksas.Circle.Get.xy(r, angle2)
  {end_x, end_y} = Cfsjksas.Circle.Get.xy(r, angle1)
  {beg_x, beg_y, end_x, end_y}
end
