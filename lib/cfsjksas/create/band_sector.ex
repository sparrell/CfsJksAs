defmodule Cfsjksas.Create.BandSector do
  @moduledoc """
  ...
  * make_shape - makes the sector shape; calls:
  ...
  A band has 2**band sectors, numbered from the x-axis.
  A sector is identified by the band and the sector number.
  ...
  Point A:
    ...
    - radians = (sector number + 1 mod band**2) * 2pi / 2**band
    - A and C have same radians
  Point B:
    - 'lower' end on 'outer arc'
    - 'outside' end of 'lower' ray
    - radians = (sector number) * 2pi / 2**band
...
def make_shape(band, sector_num, line_color, fill, fill_opacity) do
  # make sector
  ## inputs: band, sector, line_color, fill
  ### line_color \\ "black", fill \\ "none", fill_opacity \\ "50%"

  new_struct(band, sector_num, line_color, fill, fill_opacity)
  |> add_id()
  |> add_quadrant()
  |> add_reverse()
  |> add_inner_radius()
...
defp new_struct(band, sector_num, line_color, fill, fill_opacity) do
  %Cfsjksas.Create.Sector{band: band, sector_num: sector_num,
...
defp add_id(sector) do
  id = "sector_" <> to_string(sector.band) <> "_" <> to_string(sector.sector_num)
  %Cfsjksas.Create.Sector{sector | id: id}
end

defp add_quadrant(sector) do
  total_sectors = 2 ** sector.band
  quadrant = cond do
...
defp add_inner_outer_radii(sector) do
  {inner, outer} = Cfsjksas.Create.Get.annulus(sector.band)
end

defp add_inner_radius(sector) do

  inner_radius = case sector.band do
    0 ->
      0
    _ ->
      Cfsjksas.Create.Get.radius(sector.band - 1)
  end
  %Cfsjksas.Create.Sector{sector | inner_radius: inner_radius }
end

defp add_outer_radius(sector) do
  outer_radius = Cfsjksas.Create.Get.radius(sector.band)
  %Cfsjksas.Create.Sector{sector | outer_radius: outer_radius }
end

defp add_lower_radians(sector) do
  lower_radians = sector.sector_num * 2 * :math.pi() / (2 ** sector.band)
  %Cfsjksas.Create.Sector{sector | lower_radians: lower_radians }
end

defp add_upper_radians(sector) do
  upper_radians = rem(sector.sector_num + 1, 2**sector.band) * 2 * :math.pi() / (2 ** sector.band)
...
defp add_svg(sector) when sector.band == 0 do
  IO.inspect("starting add_svg band=0")

  # band 0 is special since not a sector but a circle

  {ctr_x, ctr_y} = Cfsjksas.Create.Get.center
  r = Cfsjksas.Create.Get.radius(sector.band)

  # get necessary info
  config = Cfsjksas.Create.Get.config(sector.band)
...
def add_name(band, person, sector, :one_arc) do
# all on one arc line
...
id = "arc_g" <> to_string(band) <> "_s" <> to_string(person.sector)
...
font_size = Enum.at(Cfsjksas.Create.Get.config(band).font_sizes, 0)

make_hidden_arc(id, r, beg_x, beg_y, end_x, end_y, sweep)
<> make_name_text(id, font_size, text)

end
def add_name(band, person, sector, :two_arcs) do
...
id_n = "arc_g" <> to_string(band) <> "_s" <> to_string(person.sector) <> "_n"
id_d = "arc_g" <> to_string(band) <> "_s" <> to_string(person.sector) <> "_d"

# draw the two arcs 1/3 and 2/3 of the way between the inner and outer arcs
# sweep is dependent on quadrant
sweep = Cfsjksas.Create.Get.quadrant_sweep(sector.quadrant)
# diff font sizes for name and date
config = Cfsjksas.Create.Get.config(band)
...
def add_name(band, person, sector, :three_arcs) do
...
# make hidden arcs to put text on
id_g = "arc_g" <> to_string(band) <> "_s" <> to_string(person.sector) <> "_g"
id_s = "arc_g" <> to_string(band) <> "_s" <> to_string(person.sector) <> "_s"
id_d = "arc_g" <> to_string(band) <> "_s" <> to_string(person.sector) <> "_d"
...
def add_name(band, person, sector, :two_rays) do
  ...
  id_1 = "ray_g" <> to_string(band) <> "_s" <> to_string(person.sector) <> "_1"
  ...
  id_2 = "ray_g" <> to_string(band) <> "_s" <> to_string(person.sector) <> "_2"
...
def add_name(band, person, sector, :one_ray) do
...
id = "ray_g" <> to_string(band) <> "_s" <> to_string(person.sector)
